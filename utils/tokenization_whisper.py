...
class WhisperTokenizer(PreTrainedTokenizer):
    ...
    def prefix_tokens(self) -> List[int]:
        ...
        bos_token_id = all_special_ids[-110] if len(all_special_ids) >= 110 else all_special_ids[-106]
        ...
        if self.language is not None:
            if language_id in ['cv', 'ug', 'sh', 'ky']:
                add  = 7
            else: add = 1
            bos_sequence.append(bos_token_id + add + langs.index(language_id))
        if self.task is not None:
